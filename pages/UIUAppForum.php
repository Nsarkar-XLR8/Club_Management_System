<?php
// pages/UIUAppForum.php - FINAL VERSION with User-Designed Cards
...
$memberCount = 0;
if ($stmt = $conn->prepare("SELECT COUNT(*) FROM app_forum_members WHERE club_id = ?")) {
...
$sql = "SELECT u.user_id, u.name, u.email, afm.position, afm.joined_at
...
if (!empty($searchQuery)) {
    ...
    $sql .= " AND (u.name LIKE ? OR u.email LIKE ? OR afm.position LIKE ?)";
    ...
    $sql .= " ORDER BY CASE 
                WHEN afm.position LIKE 'President' THEN 1
                WHEN afm.position LIKE 'Vice President' THEN 2
                WHEN afm.position LIKE '%Coordinator%' THEN 3
                WHEN afm.position LIKE '%Head%' THEN 4
                ELSE 5
              END, u.name ASC";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $clubId);
...
<head>
  ...
  <title>UIU APP Forum - The Official Hub</title>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .club-hero {
        background: linear-gradient(45deg, rgba(15, 23, 42, 0.9), rgba(37, 99, 235, 0.9)), url('../images/club_banner.jpg') no-repeat center center/cover;
...
<body>
  ...
  <section class="club-hero">
    <h1>UIU APP Forum</h1>
    <p>Innovate, Collaborate, and Code. Join our community of developers and tech enthusiasts to build the future, one app at a time.</p>
  ...
  <main class="page-container">
    <div class="main-content">
      <div class="section-header">
        <h2>Our Members</h2>
        <form method="GET" action="UIUAppForum.php" class="search-bar">
  ...
  <footer>
    <p>&copy; <?php echo date('Y'); ?> UIU APP Forum | All Rights Reserved</p>
  </footer>
