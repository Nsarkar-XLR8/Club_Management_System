<?php
// AddEventComputerClub.php — With modern design and Remove Event functionality
include('../db.php');
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'remove') {
            ...
            if ($event_id_to_remove) {
                ...
                if ($row = $result->fetch_assoc()) {
                    ...
                    // Delete the image file if it's not the default one and exists
                    if ($imagePathToDelete && strpos($imagePathToDelete, 'default_event.png') === false) {
                        @unlink($imagePathToDelete); // Use @ to suppress warnings if file is already gone
                    }
                }
                ...
                $stmt_delete->close();
                header("Location: AddEventComputerClub.php");
                exit;
            }
        }
        // --- HANDLE EVENT ADDITION ---
        // The form submit button must have name="action" value="add"
...
<head>
  ...
  <title>Add Event — Computer Club</title>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .nav-link.logout { background: transparent; width: 100%; border: none; font-family: inherit; font-size: inherit; text-align: left; }
...
<a class="nav-link <?php echo basename($_SERVER['PHP_SELF'])==='AddMemberRoboticClub.php'?'active':''; ?>" href="AddMemberRoboticClub.php">
  ...
  <div class="content-wrap">
    <header class="topbar">
        <h1>Add New Computer Club Event</h1>
              ...
              <ul class="upcoming-list">
                <?php if (!empty($upcoming)): foreach ($upcoming as $ev): ?>
                  <li class="up-item">
                    ...
                    <form method="POST" action="AddEventComputerClub.php" onsubmit="return confirm('Are you sure you want to remove this event?');">
        ...
        <section>
            <div class="card">
                ...
                <div class="card-body">
                  ...
                  <?php endif; ?>
                  <form method="POST" action="AddEventComputerClub.php" enctype="multipart/form-data" class="form-grid" novalidate>
                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($CSRF); ?>">
                    <div>
                      <label for="titleInput">Title</label>
                      <input type="text" name="title" id="titleInput" required placeholder="e.g., Annual Programming Contest">
                    </div>
                    <div>
