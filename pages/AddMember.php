<?php
// AddMember.php — Hardened with Remove Member functionality
include('../db.php');
...
$clubId = 1; // UIU App Forum club_id
$message = "";
    ...
    if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'remove') {
            ...
            if ($user_id_to_remove) {
                $stmt = $conn->prepare("DELETE FROM app_forum_members WHERE user_id = ? AND club_id = ?");
        ...
        // --- HANDLE MEMBER ADDITION ---
            ...
            if (empty($name) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                try {
                    $user_id = null;
                    $stmt_check = $conn->prepare("SELECT user_id, name, role FROM users WHERE email = ?");
                    if (!$stmt_check) throw new Exception("Database prepare error (check user)");
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        if ($existing_user['name'] !== $name || $existing_user['role'] !== $role) {
                            $stmt_update = $conn->prepare("UPDATE users SET name = ?, role = ? WHERE user_id = ?");
                            $stmt_update->bind_param("ssi", $name, $role, $user_id);
                            $stmt_update->execute();
                            $stmt_update->close();
                        }
                    } else {
                        $stmt_insert = $conn->prepare("INSERT INTO users (name, email, role) VALUES (?, ?, ?)");
                    ...
                    $stmt_check_member = $conn->prepare("SELECT user_id FROM app_forum_members WHERE user_id = ? AND club_id = ?");
                    $stmt_check_member->bind_param("ii", $user_id, $clubId);
                    $stmt_check_member->execute();
                    
                    if ($stmt_check_member->get_result()->num_rows > 0) {
                        $message = "⚠️ This user is already a member of the App Forum.";
                    } else {
                        $joined_at = date("Y-m-d H:i:s");
                        $stmt_add_member = $conn->prepare("INSERT INTO app_forum_members (user_id, club_id, joined_at) VALUES (?, ?, ?)");
                        $stmt_add_member->bind_param("iis", $user_id, $clubId, $joined_at);
        ...
        // Regenerate CSRF token after every successful POST action
        $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
        $CSRF = $_SESSION['csrf_token'];
    }
}
...
// Fetch recent members for the sidebar list
$recentMembers = [];
$sql_recent = "SELECT u.user_id, u.name, u.email, u.role 
               FROM app_forum_members afm
               JOIN users u ON afm.user_id = u.user_id
               WHERE afm.club_id = ?
               ORDER BY afm.joined_at DESC LIMIT 5";
                ...
                <?php if (!empty($recentMembers)): foreach ($recentMembers as $member): ?>
                  <li class="member-item">
                    ...
                    <div class="member-details">
                      ...
                      <div class="member-info"><?php echo htmlspecialchars($member['role'] ?: 'Member'); ?></div>
                    </div>
                    <form method="POST" action="AddMember.php" onsubmit="return confirm('Are you sure you want to remove this member from the club?');">
        ...
        <section>
            <div class="card">
                ...
                <div class="card-body">
                  ...
                  <form method="POST" action="AddMember.php" class="form-grid" novalidate>
                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($CSRF); ?>">
                    <div>
                      <label for="nameInput">Full Name</label>
                      <input type="text" name="name" id="nameInput" required placeholder="e.g., Jane Doe">
                    </div>
                    <div>
                      <label for="emailInput">Email Address</label>
                      <input type="email" name="email" id="emailInput" required placeholder="e.g., jane.doe@example.com">
                    </div>
                    <div>
                      <label for="roleInput">Role / Position <span class="muted">(Optional)</span></label>
                      <input type="text" name="role" id="roleInput" placeholder="e.g., Coordinator, Member">
                    </div>
                    <div style="margin-top: 12px; text-align: right;">
                      <button type="submit" class="btn-primary"><i class="fas fa-user-plus"></i> Add Member</button>
                    </div>
                  </form>
                </div>
            </div>
