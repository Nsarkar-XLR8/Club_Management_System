<?php
// pages/UIUComputerClub.php - FINAL VERSION with User-Designed Cards
if (session_status() === PHP_SESSION_NONE) session_start();
include realpath(__DIR__ . '/../db.php');

$clubId = 2; // UIU Computer Club

// --- Fetch Total Member Count ---
$memberCount = 0;
if ($stmt = $conn->prepare("SELECT COUNT(*) FROM computer_club_members WHERE club_id = ?")) {
...
$sql = "SELECT u.user_id, u.name, u.email, ccm.position, ccm.joined_at
        FROM computer_club_members ccm
        JOIN users u ON ccm.user_id = u.user_id
        WHERE ccm.club_id = ?";

if (!empty($searchQuery)) {
    $searchTerm = "%{$searchQuery}%";
    $sql .= " AND (u.name LIKE ? OR u.email LIKE ? OR ccm.position LIKE ?)";
    ...
    $sql .= " ORDER BY CASE 
                WHEN ccm.position LIKE 'President' THEN 1
                WHEN ccm.position LIKE 'Vice President' THEN 2
                WHEN ccm.position LIKE '%Coordinator%' THEN 3
                WHEN ccm.position LIKE '%Head%' THEN 4
                ELSE 5
              END, u.name ASC";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $clubId);
...
// --- Fetch Upcoming Events (from the unified 'events' table) ---
$upcomingEvents = [];
...
// --- Fetch Recent Events (from the unified 'events' table) ---
$recentEvents = [];
...
<head>
  ...
  <title>UIU Computer Club - The Official Hub</title>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .club-hero {
        background: linear-gradient(45deg, rgba(15, 23, 42, 0.9), rgba(0, 119, 255, 0.9)), url('../images/club_banner_computer.jpg') no-repeat center center/cover;
...
<body>
  ...
  <section class="club-hero">
    <h1>UIU Computer Club</h1>
    <p>Dive into the world of computing, from competitive programming to cutting-edge AI. Join us to build skills and connect with fellow tech innovators.</p>
  ...
  <main class="page-container">
    <div class="main-content">
      <div class="section-header">
        <h2>Club Members</h2>
        <form method="GET" action="UIUComputerClub.php" class="search-bar">
    ...
    <aside class="sidebar-content">
      <div class="sidebar-section">
        <h2><i class="fas fa-calendar-alt" style="color: var(--primary-color); margin-right: 8px;"></i>Upcoming Events</h2>
      ...
      <div class="sidebar-section">
        <h2><i class="fas fa-history" style="color: var(--primary-color); margin-right: 8px;"></i>Recent Events</h2>
        <div class="events-list">
          <?php if (!empty($recentEvents)): ?>
            ...
            <?php endforeach; ?>
          <?php else: ?>
            <p style="color: var(--muted-text);">No recent events to show.</p>
          <?php endif; ?>
        </div>
      </div>
    </aside>
  ...
  <footer>
    <p>&copy; <?php echo date('Y'); ?> UIU Computer Club | All Rights Reserved</p>
  </footer>
