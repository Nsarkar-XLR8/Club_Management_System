<?php
// AddMemberComputerClub.php — With modern design and Remove Member functionality
include('../db.php');
    ...
    if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'remove') {
            ...
            if ($user_id_to_remove) {
                $stmt = $conn->prepare("DELETE FROM computer_club_members WHERE user_id = ? AND club_id = ?");
                $stmt->bind_param("ii", $user_id_to_remove, $clubId);
                if ($stmt->execute()) {
                    $_SESSION['flash_message'] = "✅ Member removed from Computer Club successfully.";
                } else {
                    $_SESSION['flash_message'] = "❌ Error removing member.";
                }
                $stmt->close();
                header("Location: AddMemberComputerClub.php");
                exit;
            }
        }
        ...
        elseif (isset($_POST['action']) && $_POST['action'] === 'add') {
            ...
            if (empty($name) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                try {
                    ...
                    $stmt_check_member = $conn->prepare("SELECT user_id FROM computer_club_members WHERE user_id = ? AND club_id = ?");
                    $stmt_check_member->bind_param("ii", $user_id, $clubId);
                    $stmt_check_member->execute();
                    
                    if ($stmt_check_member->get_result()->num_rows > 0) {
                        $message = "⚠️ This user is already a member of the Computer Club.";
                    } else {
                        $joined_at = date("Y-m-d H:i:s");
                        $stmt_add_member = $conn->prepare("INSERT INTO computer_club_members (user_id, club_id, position, joined_at) VALUES (?, ?, ?, ?)");
...
$recentMembers = [];
$sql_recent = "SELECT u.user_id, u.name, ccm.position 
               FROM computer_club_members ccm
               JOIN users u ON ccm.user_id = u.user_id
               WHERE ccm.club_id = ?
               ORDER BY ccm.joined_at DESC LIMIT 5";
...
<head>
  ...
  <title>Add Member — Computer Club</title>
...
<a class="nav-link <?php echo basename($_SERVER['PHP_SELF'])==='AddMemberRoboticClub.php'?'active':''; ?>" href="AddMemberRoboticClub.php">
  ...
  <div class="content-wrap">
    <header class="topbar">
        <h1>Add New Computer Club Member</h1>
        <a href="UIUComputerClub.php" class="muted" style="text-decoration:none; display:inline-flex; align-items:center; gap: 8px;">
              ...
              <ul class="member-list">
                <?php if (!empty($recentMembers)): foreach ($recentMembers as $member): ?>
                  <li class="member-item">
                    ...
                    <form method="POST" action="AddMemberComputerClub.php" onsubmit="return confirm('Are you sure you want to remove this member from the club?');">
        ...
        <section>
            <div class="card">
                ...
                <div class="card-body">
                  ...
                  <form method="POST" action="AddMemberComputerClub.php" class="form-grid" novalidate>
                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($CSRF); ?>">
                    <div>
                      <label for="nameInput">Full Name</label>
                      <input type="text" name="name" id="nameInput" required placeholder="e.g., John Doe">
                    </div>
                    <div>
                      <label for="emailInput">Email Address</label>
                      <input type="email" name="email" id="emailInput" required placeholder="e.g., john.doe@example.com">
                    </div>
                    <div>
                      <label for="positionInput">Position in Club</label>
                      <input type="text" name="position" id="positionInput" required placeholder="e.g., President, Member">
                    </div>
